...
function StatusView({ accounts, providers, runtime, onTestAccount, testingAccountId, testResults, reminderRules, mode = 'rings', onModeChange, onOpenSettings }) {
  const groups = { active: accounts.filter((a) => a.status === 'active'), warning: accounts.filter((a) => a.status === 'warning') };
  const viewControls = <div className="overview-controls" aria-label="账号总览展示方式"><button className={mode === 'rings' ? 'active' : ''} onClick={() => onModeChange('rings')} title="同心圆总览" aria-label="同心圆总览"><CircleGauge size={16} /></button><button className={mode === 'rows' ? 'active' : ''} onClick={() => onModeChange('rows')} title="按账号行展示" aria-label="按账号行展示"><Rows3 size={16} /></button><button className={mode === 'periods' ? 'active' : ''} onClick={() => onModeChange('periods')} title="按时间周期展示" aria-label="按时间周期展示"><Clock3 size={16} /></button><button onClick={onOpenSettings} title="设置" aria-label="打开设置"><Settings2 size={16} /></button></div>;
  if (mode === 'rows') return <div className="view-stack"><section className="view-intro overview-title"><h1>账号总览</h1>{viewControls}</section><WindowsView accounts={accounts} providers={providers} reminderRules={reminderRules} embedded /></div>;
  if (mode === 'periods') return <div className="view-stack"><section className="view-intro overview-title"><h1>账号总览</h1>{viewControls}</section><PriorityView accounts={accounts} providers={providers} reminderRules={reminderRules} /></div>;
  return <div className="view-stack">
    <section className="view-intro overview-title"><h1>账号总览</h1><div className="overview-title-right"><div className="health-summary"><ShieldCheck size={17} /><strong>{groups.active.length}</strong><span>个账号正常</span></div>{viewControls}</div></section>
...
function WidgetRow({ account, provider, compact = false, tagLimit = 2, onDoubleClick }) {
  ...
  return <div className={`widget-row ${compact ? 'compact' : ''}`} title={account.lastError || account.name} onDoubleClick={onDoubleClick}><Logo provider={provider} size="sm" /><div className="widget-account-block"><span className="widget-account-marquee"><span className="widget-account">{account.name}</span></span>{tagLimit > 0 && <span className="widget-tags">{(account.tags || []).slice(0, 1).map((tag) => <em key={tag}>{tag}</em>)}</span>}</div><div className="widget-meters">{visibleMeters.length ? visibleMeters.map((meter) => <span className={`widget-meter ${meter.available === false ? 'off' : ''}`} key={meter.key}><b>{windowCatalog[meter.key]?.short}</b><em>{formatAmount(meter)}</em><small>{formatResetCompact(meter.resetAt)}</small></span>) : <span className="widget-empty">等待同步</span>}</div><span className={`widget-live ${account.status === 'warning' ? 'warning' : ''}`}><i /></span></div>;
}
