...
// 浮窗大小档位与主进程 WIDGET_SIZES 对应，内容按 zoom 缩放
const WIDGET_ZOOM = { small: 0.85, medium: 1, large: 1.2 };
const normalizeSettings = (value = {}) => ({
  alerts: true, pollMinutes: '15', widgetTagLimit: '2', reminderRules: defaultReminderRules,
  widget: true, widgetPreview: false, theme: 'dark', widgetSize: 'medium',
  ...value,
  reminderRules: Array.isArray(value.reminderRules) ? value.reminderRules : defaultReminderRules,
  theme: value.theme === 'light' ? 'light' : 'dark',
  widgetSize: WIDGET_ZOOM[value.widgetSize] ? value.widgetSize : 'medium',
});
...
function SettingsDrawer({ accounts, providers, settings, setSettings, onClose, openModal, onDeleteAccount, onTestAccount, testingAccountId, onEditProvider, autoLaunch, onToggleAutoLaunch, appVersion, update, onOpenUpdate }) {
  return <><div className="drawer-shade" onClick={onClose} /><aside className="settings-drawer" aria-label="设置">
    <div className="drawer-scroll">
      ...
      <section className="drawer-section"><div className="drawer-section-title"><Monitor size={16} /><span>桌面浮窗</span></div><Toggle checked={settings.widget} onChange={(value) => setSettings((old) => ({ ...old, widget: value }))} label="显示桌面浮窗" description="固定在桌面顶层，双击展开主窗口" /><div className="setting-select"><span><b>浮窗大小</b><small>调整浮窗与文字尺寸</small></span><select value={WIDGET_ZOOM[settings.widgetSize] ? settings.widgetSize : 'medium'} onChange={(event) => setSettings((old) => ({ ...old, widgetSize: event.target.value }))}><option value="small">小</option><option value="medium">中</option><option value="large">大</option></select></div><div className="widget-setting-preview"><WidgetRow account={accounts[0]} provider={providers.find((item) => item.id === accounts[0]?.providerId)} compact tagLimit={Number(settings.widgetTagLimit ?? 2)} /></div><button className="outline-button full" onClick={() => setSettings((old) => ({ ...old, widgetPreview: true }))}><Eye size={15} /> 预览并调整</button></section>
      ...
      <section className="drawer-section"><div className="drawer-section-title"><LayoutGrid size={16} /><span>厂商适配器</span><button className="mini-add" onClick={() => openModal('provider')}><Plus size={14} /> 新增厂商</button></div><div className="settings-list providers-list">{providers.map((provider) => <div className="settings-account" key={provider.id}><Logo provider={provider} size="sm" /><div><b title={provider.name}>{provider.name}</b><small>{provider.requestConfig?.adapterMode === 'script' ? '脚本适配' : '标准映射'}</small></div><button className="row-icon-button" title="编辑厂商" aria-label={`编辑 ${provider.name}`} onClick={() => onEditProvider(provider)}><Pencil size={13} /></button><span className="adapter-state"><Check size={13} /></span></div>)}</div></section>
...
function WidgetPreview({ account, provider, onClose, tagLimit = 2, zoom = 1 }) {
  return <div className="widget-preview-layer"><div className="widget-preview-head"><span><Monitor size={14} /> 浮窗预览</span><button className="icon-button" onClick={onClose} aria-label="关闭预览"><X size={15} /></button></div><div style={{ zoom }}><WidgetRow account={account} provider={provider} compact tagLimit={tagLimit} /></div></div>;
}
...
function WidgetApp() {
  ...
  return <div className="widget-window-shell" style={{ zoom: WIDGET_ZOOM[settings.widgetSize] || 1 }} title="拖动移动，双击展开，滚轮切换账号" onWheel={(event) => cycleAccount(event.deltaY > 0 ? 1 : -1)} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={stopDrag} onPointerCancel={stopDrag} onDoubleClick={() => { if (!dragRef.current.moved) window.quotaDesk?.openMainWindow(); }}><WidgetRow account={account} provider={provider} compact tagLimit={Number(settings.widgetTagLimit ?? 2)} /></div>;
}
...
function App() {
  ...
  useEffect(() => { if (bridge && hydrated.current) bridge.setWidgetSize?.(settings.widgetSize).catch(() => {}); }, [settings.widgetSize]);
  ...
  return <div className="app-shell">
    ...
    {settings.widgetPreview && <WidgetPreview account={currentWidgetAccount} provider={currentWidgetProvider} tagLimit={Number(settings.widgetTagLimit ?? 2)} zoom={WIDGET_ZOOM[settings.widgetSize] || 1} onClose={() => setSettings((old) => ({ ...old, widgetPreview: false }))} />}
